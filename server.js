...
const FRONTEND = process.env.FRONTEND_URL || 'https://atas-doener-pizza.com';
...
app.get('/api/status', async (req, res) => {
  try {
    ...
    // lieferung und lieferAb kommen aus der Uhr, nicht aus der Datenbank: sie
    // sind eine Eigenschaft der Tageszeit, kein Zustand, den jemand setzt.
    const lf = calcLieferfenster();
    res.json({ mode, isOpen: mode === 'online', lieferung: lf.offen, lieferAb: lf.ab, lieferGrund: lf.grund });
  } catch(e) { res.json({ mode: 'online', isOpen: true, lieferung: true, lieferAb: null, lieferGrund: null }); }
});
...
// ── Rabattaktion: serverseitig nachrechnen ───────────────────────
// ATAS fuehrt derzeit keine Aktion. RABATT_AKTIV ist der Schalter dafuer; die
// Rechenlogik bleibt stehen, damit eine spaetere Aktion nur zwei Werte braucht
// statt eines Umbaus. Code, Prozentsatz und Mindestwert muessen dann mit der
// Kopie im Frontend uebereinstimmen - der Server rechnet nach und gewinnt,
// aber eine abweichende Anzeige verspricht dem Gast etwas Falsches.
const RABATT_AKTIV       = false;
// Der Client darf einen Betrag vorschlagen – maßgeblich ist allein diese Rechnung.
const RABATT_CODE        = 'ATAS-KEIN-RABATT';   // Platzhalter, solange RABATT_AKTIV false ist
...
app.post('/api/orders', async (req, res) => {
  try {
    ...
    // Lieferung ausserhalb des Lieferfensters ablehnen, noch vor jedem
    // Datenbankzugriff: die Pruefung braucht nur die Uhr, und eine Absage soll
    // auch dann klar herauskommen, wenn die Datenbank gerade klemmt.
    //
    // Die Sperre im Frontend allein genuegt nicht: eine seit dem Mittag offene
    // Seite kennt das Fenster nicht mehr und wuerde trotzdem abschicken. Die
    // Kasse ist ausgenommen - am Telefon entscheidet die Kueche, nicht die Uhr.
    if (!isPOS && req.body.mode === 'lieferung') {
      const lf = calcLieferfenster();
      if (!lf.offen) {
        const texte = {
          zu_frueh:   `Wir liefern heute erst ab ${lf.ab} Uhr. Abholung ist schon jetzt moeglich.`,
          feierabend: 'Wir haben fuer heute geschlossen. Bestellungen sind ab der naechsten Oeffnung wieder moeglich.',
          ruhetag:    'Heute ist Ruhetag. Wir liefern nicht und haben auch nicht geoeffnet.',
  ...
  try {
    ...
    const ppOrder = await paypalApi('/v2/checkout/orders', 'POST', token, {
      ...
      purchase_units: [{
        ...
        description:  `ATAS Döner & Pizza Bestellung #${orderNum}`,
        amount: { currency_code: 'EUR', value: Number(total).toFixed(2) }
      }],
      application_context: {
        brand_name:          'ATAS Döner & Pizza',
        locale:              'de-DE',
        user_action:         'PAY_NOW',
...
async function sendConfirmationEmail(order, mins) {
  ...
  try {
    await getResend()?.emails.send({
      from: process.env.EMAIL_FROM || 'bestellungen@atas-doener-pizza.com',
      to:   order.customer.email,
      subject: `✅ Bestellung #${order.orderNum} bestätigt – ATAS Döner & Pizza`,
      html: `<div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto">
        <div style="background:#e8a020;color:#fff;padding:22px;text-align:center">
          <h1 style="margin:0;font-size:22px">🍕 ATAS Döner & Pizza</h1>
          <p style="margin:4px 0 0;opacity:.8;font-size:13px">Nordstr. 45a · 59269 Beckum</p>
        </div>
        <div style="padding:26px 22px">
          <h2 style="color:#e8a020;margin:0 0 14px">Bestellung #${order.orderNum} bestätigt ✅</h2>
          ...
          <div style="background:#fff8f0;border-left:4px solid #d4a76a;padding:12px 16px;border-radius:0 8px 8px 0;margin:14px 0">
            ...
            <p style="margin:4px 0 0;font-size:15px;font-weight:bold;color:#e8a020">⏱ Voraussichtlich ~${m} Minuten</p>
          </div>
          ...
          <div style="border-top:1px solid #eee;padding-top:10px;font-size:13px">
            ...
            ${order.discount>0?`<div style="display:flex;justify-content:space-between;color:#2e7d32;margin:3px 0"><span>Rabatt</span><span>\u2212${order.discount.toFixed(2)} \u20ac</span></div>`:''}
            ...
            <div style="display:flex;justify-content:space-between;font-weight:bold;font-size:15px;border-top:2px solid #e8a020;padding-top:8px;margin-top:6px"><span>Gesamt</span><span style="color:#e8a020">${(order.total||0).toFixed(2).replace('.',',')} €</span></div>
          </div>
        ...
        <div style="background:#f7f3ee;padding:14px;text-align:center;font-size:11px;color:#999">ATAS Döner & Pizza · Nordstr. 45a · 59269 Beckum · Tel: 02521 / 826 48 00</div>
      </div>`
    });
...
async function sendCancellationEmail(order, reason, refundStatus) {
  ...
  try {
    await getResend()?.emails.send({
      ...
      html: `<div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto">
        <div style="background:#e8a020;color:#fff;padding:22px;text-align:center"><h1 style="margin:0">🍕 ATAS Döner & Pizza</h1></div>
        <div style="padding:26px 22px">
          ...
          ${refundHtml}
          <p>Bei Fragen: <strong>02521 / 826 48 00</strong></p>
        </div>
      </div>`
    });
...
// einmal im Jahr zu ergaenzen ist billiger und nachlesbar. Bleibt sie leer,
// laeuft ein Feiertag wie sein Wochentag - falsch, aber harmlos.
const FEIERTAGE = [
  // 'TT.MM.JJJJ', NRW. Beispiel: '03.10.2026'
];

// Wochentag (0 = So) -> [von, bis] in Minuten seit Mitternacht.
// Der Dienstag fehlt bewusst: Ruhetag.
const OEFFNUNG = {
  0: [12 * 60,      22 * 60],       // Sonntag
  1: [11 * 60 + 30, 21 * 60 + 30],  // Montag
  3: [11 * 60 + 30, 21 * 60 + 30],  // Mittwoch
  4: [11 * 60 + 30, 21 * 60 + 30],  // Donnerstag
  5: [11 * 60 + 30, 22 * 60],       // Freitag
...
// abgeschrieben und beim naechsten Zeitzonenfehler nur eine Kopie repariert.
function deutscheZeit() {
  const roh = new Date().toLocaleString('de-DE', { timeZone: 'Europe/Berlin' });
  const p = roh.match(/(\d+)\.(\d+)\.(\d+),\s*(\d+):(\d+)/);
  if (!p) return null;
  const tag = parseInt(p[1]), mon = parseInt(p[2]), jahr = parseInt(p[3]);
  const zwei = n => String(n).padStart(2, '0');
  return {
    wd:    new Date(jahr, mon - 1, tag).getDay(),
    mins:  parseInt(p[4]) * 60 + parseInt(p[5]),
    datum: `${zwei(tag)}.${zwei(mon)}.${jahr}`,
  };
}

// Wochentag nach Feiertagsregel: ein Feiertag laeuft wie ein Sonntag.
function wochentagMitFeiertag(t) {
  return FEIERTAGE.includes(t.datum) ? 0 : t.wd;
}

function calcAutoMode() {
  const t = deutscheZeit();
  if (!t) return 'geschlossen';
  const fenster = OEFFNUNG[wochentagMitFeiertag(t)];
  if (!fenster) return 'geschlossen';
  return (t.mins >= fenster[0] && t.mins < fenster[1]) ? 'online' : 'geschlossen';
}

// Liefert, ob gerade ausgeliefert wird, und ab wann sonst. Nie ueber den
// Ladenschluss hinaus: wer zu hat, liefert auch nicht.
function calcLieferfenster() {
  const t = deutscheZeit();
  if (!t) return { offen: false, ab: null, grund: 'unbekannt' };
  const wd = wochentagMitFeiertag(t);
  const fenster = OEFFNUNG[wd];
  if (!fenster) return { offen: false, ab: null, grund: 'ruhetag' };
  const ab = (wd === 0 || wd === 6) ? LIEFER_AB_WOCHENENDE : LIEFER_AB_WERKTAG;
  const alsUhr = m => `${String(Math.floor(m / 60)).padStart(2, '0')}:${String(m % 60).padStart(2, '0')}`;
  // "noch nicht" und "nicht mehr" auseinanderhalten. Sonst steht um 23:41 Uhr
  // die Meldung "wir liefern erst ab 16:00" da, und die ist schlicht falsch.
  if (t.mins < ab)          return { offen: false, ab: alsUhr(ab), grund: 'zu_frueh' };
  if (t.mins >= fenster[1]) return { offen: false, ab: alsUhr(ab), grund: 'feierabend' };
  return { offen: true, ab: alsUhr(ab), grund: null };
}
...
const PDF_FT = 810;
const PDF_SV = 0.99;
const PDF_BASE = 0;   // keine monatliche Grundgebuehr - ATAS zahlt nur die Servicegebuehr je Bestellung
const pdfFmt = n => n.toFixed(2).replace('.', ',') + ' €';

function pdfColorBox(doc, title, sub, color = '#e8a020', h = 70) {
...
function pdfKundenliste(doc, orders) {
  ...
  drawGroup('Barzahlung', '#e8a020', orders.filter(o => o.payment === 'bar'));
...
function pdfBarRechnung(doc, barOrders, barStats, zeitraum) {
  ...
  doc.font('Helvetica').fontSize(9).fillColor('#444').text('ATAS Döner & Pizza', PDF_M+270, addrY+12).text('Nordstr. 45a, 59269 Beckum', PDF_M+270, addrY+22);
  doc.y = addrY + 38;
...
cron.schedule('0 22 * * *', async () => {
  try {
    ...
    const tagespdf = await generatePdf(doc => {
      ...
      // Header
      doc.rect(0,0,595,70).fill('#e8a020');
      ...
      doc.fontSize(10).font('Helvetica').fillColor('rgba(255,255,255,0.8)')
        .text(`ATAS Döner & Pizza  ·  ${label}`, 50, 44);
      ...
      // Gesamtumsatz
      const ty = doc.y;
      doc.rect(50,ty,W,36).fill('#e8a020');
      ...
      // Bestelldetails
      doc.font('Helvetica-Bold').fontSize(13).fillColor('#e8a020').text('Alle Bestellungen');
      doc.moveDown(0.4);
      ...
      orders.forEach((o, i) => {
        ...
        doc.font('Helvetica-Bold').fontSize(10).fillColor('#e8a020')
      ...
      doc.fontSize(8).fillColor('#aaa')
        .text(`ATAS Döner & Pizza  ·  Tagesbericht ${label}  ·  Erstellt: ${now.toLocaleTimeString('de-DE')}`, 50, 790, { width: W, align:'center' });
    });

    if (process.env.RESTAURANT_EMAIL) {
      await getResend()?.emails.send({
        from: process.env.EMAIL_FROM || 'system@atas-doener-pizza.com',
        to: process.env.RESTAURANT_EMAIL,
        subject: `📋 Tagesbericht ${label} · ATAS Döner & Pizza`,
        html: `<div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;color:#222">
          <div style="background:#e8a020;padding:22px 28px;color:#fff">
          ...
          <div style="padding:24px 28px">
            <table style="width:100%;border-collapse:collapse;font-size:13px">
              ...
              <tr style="background:#e8a020"><td style="padding:10px;font-weight:bold;color:#fff;font-size:15px">Gesamtumsatz</td><td style="padding:10px;text-align:right;font-weight:bold;color:#fff;font-size:15px">${total.toFixed(2).replace('.',',')} €</td></tr>
            </table>
...
function buildWochenberichtPdf(doc, d) {
  ...
  pdfColorBox(doc, `Wochenbericht KW ${kw} / ${jahr}`, `ATAS Döner & Pizza  ·  ${vonBis}`, '#e8a020');
  pdfKacheln(doc, [
  ...
  doc.font('Helvetica-Bold').fontSize(12).fillColor('#2e7d32').text('Auszahlung an ATAS Döner & Pizza', PDF_M+10, ay+8, { width: PDF_W*0.65 });
...
async function wochenberichtVersenden(now, { nurOwner = false } = {}) {
  ...
  if (resend && process.env.RESTAURANT_EMAIL && !nurOwner) {
    const antwort = await resend.emails.send({
      from: process.env.EMAIL_FROM || 'system@atas-doener-pizza.com',
      to: process.env.RESTAURANT_EMAIL,
      subject: `📊 Wochenbericht KW ${kw} / ${now.getFullYear()} · ATAS Döner & Pizza`,
      html: `<div style="font-family:Arial,sans-serif;max-width:580px;margin:0 auto;color:#222">
<div style="background:#e8a020;padding:24px 28px;color:#fff">
...
<div style="padding:24px 28px">
  ...
  <p style="font-size:11px;color:#aaa;margin-top:8px">Anbei der Wochenbericht mit Kundenliste${barOrders.length > 0 ? ' und Bar-Übersicht' : ''}.</p>
      ...
      attachments: [{ filename: `KW${kw}_${now.getFullYear()}_ATAS_Wochenbericht.pdf`, content: berichtPdf.toString('base64') }],
    });
  ...
  if (resend && process.env.OWNER_EMAIL) {
    const antwort = await resend.emails.send({
      ...
      subject: `📊 Wochenbericht KW ${kw} · ATAS Döner & Pizza`,
      html: `<p style="font-family:Arial,sans-serif;color:#555">Anbei der Wochenbericht KW ${kw} / ${now.getFullYear()} für ATAS Döner & Pizza.</p>
      ...
      attachments: [
        { filename: `KW${kw}_${now.getFullYear()}_ATAS_Wochenbericht.pdf`, content: berichtPdf.toString('base64') },
      ],
    });
...
function buildMonatsberichtPdf(doc, d) {
  ...
  pdfColorBox(doc, `Monatsbericht ${monat}`, `ATAS Döner & Pizza  ·  ${vonBis}`, '#e8a020');
  pdfKacheln(doc, [
    ['Bestellungen gesamt', `${orders.length}`,                                                   '#1a1a2e'],
...
cron.schedule('0 22 * * *', async () => {
  ...
  try {
    ...
    if (process.env.RESTAURANT_EMAIL) {
      await getResend()?.emails.send({
        from: process.env.EMAIL_FROM || 'system@atas-doener-pizza.com',
        to:   process.env.RESTAURANT_EMAIL,
        subject: `📅 Monatsbericht ${monat} · ATAS Döner & Pizza`,
        html: `<div style="font-family:Arial,sans-serif;max-width:580px;margin:0 auto;color:#222">
  <div style="background:#e8a020;padding:24px 28px;color:#fff">
  ...
  <div style="padding:24px 28px">
    ...
    <p style="font-size:11px;color:#aaa;margin-top:8px">Anbei der Monatsbericht mit Kundenliste${barOrdersM.length > 0 ? ' und Bar-Übersicht' : ''}.</p>
        ...
        attachments: [{ filename: `${monat.replace(' ','_')}_ATAS_Monatsbericht.pdf`, content: monatsPdf.toString('base64') }],
      });
    }
    ...
    if (process.env.OWNER_EMAIL) {
      await getResend()?.emails.send({
        ...
        to: process.env.OWNER_EMAIL,
        subject: `📅 Monatsbericht ${monat} · ATAS Döner & Pizza`,
        html: `<p style="font-family:Arial,sans-serif;color:#555">Anbei der Monatsbericht <b>${monat}</b> für ATAS Döner & Pizza.</p>
               ...
               <p style="font-family:Arial,sans-serif;color:#999;font-size:12px">Die Gebühren-Rechnung wird separat über Lexware gestellt.</p>`,
        attachments: [
          { filename: `${monat.replace(' ','_')}_ATAS_Monatsbericht.pdf`, content: monatsPdf.toString('base64') },
        ],
      });
    }
...
app.listen(PORT, () => console.log(`🍕 ATAS Döner & Pizza Backend · Port ${PORT}`));
