...
async function printOrder(order) {
  ...
  const response = await fetch('https://api.printnode.com/printjobs', {
    ...
    body: JSON.stringify({
      ...
      title: `ATAS Döner & Pizza Bestellung #${order.orderNum}`,
      contentType: 'raw_base64',
      content: base64Content,
      source: 'ATAS Döner & Pizza Backend'
    })
  });
...
function buildReceipt(order) {
  ...
  // Header
  receipt += CENTER;
  receipt += DOUBLE_ON + BOLD_ON + 'ATAS' + LF + DOUBLE_OFF + BOLD_OFF;
  receipt += 'Nordstr. 45a · 59269 Beckum' + LF;
  receipt += 'Tel: 02521 / 826 48 00' + LF;
  receipt += SEPARATOR;
